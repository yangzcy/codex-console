    ...
    return {
        "types": [
            ...
            },
            {
                "value": "yyds_mail",
                "label": "YYDS Mail",
                "description": "YYDS Mail 临时邮箱服务，使用 API Key 创建邮箱并读取邮件",
                "config_fields": [
...
async def create_email_service(request: EmailServiceCreate):
    ...
    with get_db() as db:
        ...
        service = EmailServiceModel(
            ...
            config=normalize_service_config(normalized_service_type, normalized_config),
...
async def update_email_service(service_id: int, request: EmailServiceUpdate):
    ...
    with get_db() as db:
        ...
        if request.config is not None:
            ...
            merged_config = normalize_service_config(service.service_type, merged_config)
            if (
